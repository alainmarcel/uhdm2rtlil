...
if [ ! -f "$SV_FILE" ]; then
    echo "ERROR: source file $SV_FILE (or dut.v) not found in $TEST_DIR"
    exit 1
fi

# Clean up previous runs — including any prior `slpp_all/` so Surelog
# always re-parses with the current binary.  Without this, an older
# UHDM file (possibly produced by a different Surelog build) survives
# between runs, hiding regressions and creating local-vs-CI drift
# (CI starts from a fresh checkout every time).
...
# Step 1: Generate UHDM fresh.  Some tests intentionally exercise SV
# corners that Surelog flags as errors (e.g. `(* foo *) y ++` syntax)
...
if [ $VERILOG_FAILED -eq 1 ] && [ -f "$UHDM_RTLIL" ]; then
    ...
    # Compare files
    if diff -u --label="verilog_rtlil" --label="uhdm_rtlil" "$VERILOG_RTLIL" "$UHDM_RTLIL" > rtlil_diff.txt; then
