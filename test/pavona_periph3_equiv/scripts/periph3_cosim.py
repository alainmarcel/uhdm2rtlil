...
Reuses the per-module elaboration produced by run_periph3_equiv.sh
(work/<mod>/slpp_all/surelog.uhdm) and the same periph3_srcs.py closure.

Usage: periph3_cosim.py <module> [cycles] [seed]
"""
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pavona_periph3_equiv
...
INCS   = [f"{HERE}/rtl/mbx", f"{HERE}/rtl/dma", f"{HERE}/rtl/soc_dbg_ctrl", f"{HERE}/rtl/ascon", f"{HERE}/rtl/pkg",
...
# run_periph3_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
if not os.path.isdir(WORK) or not os.path.exists(f"{WORK}/slpp_all/surelog.uhdm"):
    print(f"{mod}: NO_RUN (no elaboration; run run_periph3_equiv.sh first)")
    sys.exit(2)
os.chdir(WORK)
...
# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/periph3_srcs.py", mod]).stdout.split()
