#!/usr/bin/env python3
"""Verilator/iverilog co-sim adjudication for a Pavona small-peripheral module.
...
Reuses the per-module elaboration produced by run_periph_equiv.sh
(work/<mod>/slpp_all/surelog.uhdm) and the same periph_srcs.py closure.

Usage: periph_cosim.py <module> [cycles] [seed]
"""
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pavona_periph_equiv
...
INCS   = [f"{HERE}/rtl/rom_ctrl", f"{HERE}/rtl/uart", f"{HERE}/rtl/aon_timer", f"{HERE}/rtl/rv_timer", f"{HERE}/rtl/pattgen", f"{HERE}/rtl/sram_ctrl", f"{KMACD}/rtl/kmac", f"{ACCD}/rtl/pkg",
...
# run_periph_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
if not os.path.isdir(WORK) or not os.path.exists(f"{WORK}/slpp_all/surelog.uhdm"):
    print(f"{mod}: NO_RUN (no elaboration; run run_periph_equiv.sh first)")
    sys.exit(2)
os.chdir(WORK)
...
# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/periph_srcs.py", mod]).stdout.split()
if os.path.exists(WRAPPER):
    SR.append(WRAPPER)
