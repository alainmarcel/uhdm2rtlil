#!/usr/bin/env python3
# Print the dependency-closure .sv file list for a PERIPH top module, packages
# first (topologically), so read_slang / surelog get exactly what they need.
#
# Search roots: the six small-IP roots rtl/{keymgr_dpe,lc_ctrl,rv_dm} (hw/ip/<ip>/rtl vendored whole), PLUS the shared prim library
...
ACC  = os.path.normpath(os.path.join(HERE, '..', 'pavona_acc_equiv'))
KMAC = os.path.normpath(os.path.join(HERE, '..', 'pavona_kmac_equiv'))
KEYMGR = os.path.normpath(os.path.join(HERE, '..', 'pavona_keymgr_equiv'))
ROOTS = [f'{HERE}/rtl/keymgr_dpe', f'{HERE}/rtl/lc_ctrl', f'{HERE}/rtl/rv_dm', f'{HERE}/rtl/riscv_dbg', f'{HERE}/rtl/pkg', f'{KMAC}/rtl/kmac', f'{KEYMGR}/rtl/keymgr',
