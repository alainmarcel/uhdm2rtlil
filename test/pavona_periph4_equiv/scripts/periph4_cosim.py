...
Reuses the per-module elaboration produced by run_periph4_equiv.sh
(work/<mod>/slpp_all/surelog.uhdm) and the same periph4_srcs.py closure.

Usage: periph4_cosim.py <module> [cycles] [seed]
"""
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pavona_periph4_equiv
...
KEYMGRD = os.path.normpath(os.path.join(HERE, '..', 'pavona_keymgr_equiv'))
INCS   = [f"{HERE}/rtl/keymgr_dpe", f"{HERE}/rtl/lc_ctrl", f"{HERE}/rtl/rv_dm", f"{HERE}/rtl/riscv_dbg", f"{HERE}/rtl/pkg", f"{KMACD}/rtl/kmac", f"{KEYMGRD}/rtl/keymgr",
...
# run_periph4_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
if not os.path.isdir(WORK) or not os.path.exists(f"{WORK}/slpp_all/surelog.uhdm"):
    print(f"{mod}: NO_RUN (no elaboration; run run_periph4_equiv.sh first)")
    sys.exit(2)
os.chdir(WORK)
...
# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/periph4_srcs.py", mod]).stdout.split()
...
design -reset
read_slang --ignore-assertions -DSYNTHESIS --single-unit --relax-enum-conversions {inc_ys} {' '.join(SR)} --top {TOP}
hierarchy -check -top {TOP}
flatten; proc; memory; opt -fast; setundef -undriven -zero
delete t:$check t:$assert t:$assume t:$print
simplemap t:$bwmux
...
# ---------------------------------------------------------------- run
def run_verilator():
    # -Wno-ENUMVALUE: ascon_core assigns plain logic vectors to enum-typed
    # signals without a cast (IEEE 6.19.3); Verilator 5 rejects that even
    # under -Wno-fatal, read_slang needs --relax-enum-conversions for it.
    r = sh(["verilator", "--binary", "-j", "0", "-Wno-lint", "-Wno-style",
            "-Wno-fatal", "-Wno-ENUMVALUE", "--timing", "--no-assert", "-DSYNTHESIS", "-o", "cssim",
