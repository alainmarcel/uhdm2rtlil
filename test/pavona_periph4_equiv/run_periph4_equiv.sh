...
# rv_dm) equivalence: read_uhdm vs read_slang, SAT-mitered.  Sources are
# the per-module dependency closure (scripts/periph4_srcs.py), which pulls the ACC
# RTL plus the shared prim library / base packages vendored for the TL-UL
# campaign (../pavona_tlul_equiv/rtl/{prim,tlul,pkg}).
#   ./run_periph4_equiv.sh [module...]      # default: every manifest module
set -uo pipefail
...
INCS=(-I"$HERE/rtl/keymgr_dpe" -I"$HERE/rtl/lc_ctrl" -I"$HERE/rtl/rv_dm" -I"$HERE/rtl/riscv_dbg" -I"$HERE/rtl/pkg" -I"$KMAC/rtl/kmac" -I"$KEYMGR/rtl/keymgr" -I"$TLUL/rtl/prim" -I"$TLUL/rtl/tlul" -I"$TLUL/rtl/pkg" -I"$ACC/rtl/pkg")
...
run_one() {
  ...
  local SR; SR=$(python3 "$HERE/scripts/periph4_srcs.py" "$m")
  local top="$m"
...
MODS=(); if [ $# -gt 0 ]; then
  ...
  # there (an explicit `./run_periph4_equiv.sh aes_core` used to run with the
  # 300 s default and report the manifest's SAT-hard rows as unexpected).
  for a in "$@"; do
    e=$(awk -v m="$a" '$1==m && $1!~/^#/ {print $1":"$2":"$3":"$4; exit}' "$HERE/periph4_modules.txt")
    MODS+=("${e:-$a}")
  done
else
  while read -r n s t w _; do [ -n "${n:-}" ]&&[[ ! "$n" =~ ^# ]]&&MODS+=("$n:$s:$t:$w"); done < "$HERE/periph4_modules.txt"
fi
...
echo "PERIPH4 equivalence: $proven/$total proven, $reg unexpected"
