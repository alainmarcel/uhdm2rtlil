...
(work/<mod>/slpp_all/surelog.uhdm) and the same edn_srcs.py closure.
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pavona_edn_equiv
...
INCS   = [f"{HERE}/rtl/edn", f"{HERE}/rtl/pkg", f"{ACCD}/rtl/pkg",
...
# run_edn_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/edn_srcs.py", mod]).stdout.split()
if os.path.exists(WRAPPER):
    SR.append(WRAPPER)
...
# The two sparse FSMs park in their error state as soon as random control
# raises local_escalate_i (or drops the enable); hold those and let the
# request / ack handshake stay random.
DIRECTED = {
    "edn_ack_sm":  {"local_escalate_i": "1'b0", "enable_i": "1'b1"},
    "edn_main_sm": {"local_escalate_i": "1'b0", "edn_enable_i": "1'b1",
                    "csrng_ack_err_i": "1'b0", "boot_req_mode_i": "(i < 300)",
                    "auto_req_mode_i": "(i >= 300)"},
}
