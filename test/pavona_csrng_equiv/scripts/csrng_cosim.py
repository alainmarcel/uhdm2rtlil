...
(work/<mod>/slpp_all/surelog.uhdm) and the same csrng_srcs.py closure.
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pavona_csrng_equiv
...
INCS   = [f"{HERE}/rtl/csrng", f"{HERE}/rtl/pkg", f"{ACCD}/rtl/pkg",
...
# run_csrng_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/csrng_srcs.py", mod]).stdout.split()
if os.path.exists(WRAPPER):
    SR.append(WRAPPER)
