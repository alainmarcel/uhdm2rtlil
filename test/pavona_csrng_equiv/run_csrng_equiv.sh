#!/usr/bin/env bash
# Per-module CSRNG (Pavona OpenTitan Keccak MAC / SHA3
# hashing core) equivalence: read_uhdm vs read_slang, SAT-mitered.  Sources are
# the per-module dependency closure (scripts/csrng_srcs.py), which pulls the ACC
...
INCS=(-I"$HERE/rtl/csrng" -I"$HERE/rtl/aes" -I"$HERE/rtl/pkg" -I"$TLUL/rtl/prim" -I"$TLUL/rtl/tlul" -I"$TLUL/rtl/pkg" -I"$ACC/rtl/pkg")
...
run_one() {
  ...
  local SR; SR=$(python3 "$HERE/scripts/csrng_srcs.py" "$m")
  local top="$m"
...
MODS=(); if [ $# -gt 0 ]; then MODS=("$@"); else
  while read -r n s t w _; do [ -n "${n:-}" ]&&[[ ! "$n" =~ ^# ]]&&MODS+=("$n:$s:$t:$w"); done < "$HERE/csrng_modules.txt"
fi
...
echo "CSRNG equivalence: $proven/$total proven, $reg unexpected"
