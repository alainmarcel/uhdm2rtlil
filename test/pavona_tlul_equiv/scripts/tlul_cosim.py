...
if not clks:
    print(f"{mod}: no clocks found"); sys.exit(2)
...
SAMPLE = clks[0]   # sample on the first (host) clock
...
{seen}
  {TOP} rtl ({allck}, {conn}, {bind('r')});
  gold_{TOP} gold({allck}, {conn}, {bind('g')});
  gate_{TOP} gate({allck}, {conn}, {bind('s')});
{clkgen}
  initial begin
