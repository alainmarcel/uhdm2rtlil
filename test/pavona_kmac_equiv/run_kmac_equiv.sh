#!/usr/bin/env bash
# Per-module KMAC (Pavona OpenTitan Keccak MAC / SHA3
# hashing core) equivalence: read_uhdm vs read_slang, SAT-mitered.  Sources are
# the per-module dependency closure (scripts/kmac_srcs.py), which pulls the ACC
...
# Include dirs: KMAC-local + shared prim/tlul/base-pkg + ACC pkgs (edn/keymgr).
INCS=(-I"$HERE/rtl/kmac" -I"$HERE/rtl/pkg" -I"$TLUL/rtl/prim" -I"$TLUL/rtl/tlul" -I"$TLUL/rtl/pkg" -I"$ACC/rtl/pkg")
...
run_one() {
  ...
  local SR; SR=$(python3 "$HERE/scripts/kmac_srcs.py" "$m")
  local top="$m"
...
MODS=(); if [ $# -gt 0 ]; then MODS=("$@"); else
  while read -r n s t w _; do [ -n "${n:-}" ]&&[[ ! "$n" =~ ^# ]]&&MODS+=("$n:$s:$t:$w"); done < "$HERE/kmac_modules.txt"
fi
...
echo "KMAC equivalence: $proven/$total proven, $reg unexpected"
