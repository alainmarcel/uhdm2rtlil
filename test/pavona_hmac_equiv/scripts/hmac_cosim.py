...
(work/<mod>/slpp_all/surelog.uhdm) and the same hmac_srcs.py closure.
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pavona_hmac_equiv
...
INCS   = [f"{HERE}/rtl/hmac", f"{HERE}/rtl/pkg", f"{ACCD}/rtl/pkg",
...
# run_hmac_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/hmac_srcs.py", mod]).stdout.split()
if os.path.exists(WRAPPER):
    SR.append(WRAPPER)
