...
endpackage
module dyn_elem_member_slice_compound_write import dem_pkg::*; (input logic clk_i, input logic rst_ni, input update_e update_sel, input logic [1:0] slot_dst_sel_i, input logic cnt,
  ...
  always_comb begin
    ...
    unique case (update_sel)
      ...
      SlotLoadRoot: begin
        ...
        key_slots_d[slot_dst_sel_i].key[0] ^= root_key_i[0];
        key_slots_d[slot_dst_sel_i].key[1] ^= root_key_i[1];
        key_slots_d[slot_dst_sel_i].max_key_version = max_key_version_i;
        key_slots_d[slot_dst_sel_i].key_policy = '{allow_child: 1'b1, exportable: 1'b0};
      end
      SlotLoadFromKmac: begin
        key_slots_d[slot_dst_sel_i].valid = 1;
        key_slots_d[slot_dst_sel_i].key = kmac_data_i;
        key_slots_d[slot_dst_sel_i].max_key_version = max_key_version_i;
        key_slots_d[slot_dst_sel_i].boot_stage = (key_slots_q[0].boot_stage == BootStageCreator) ? BootStageOwner : BootStageRuntime;
