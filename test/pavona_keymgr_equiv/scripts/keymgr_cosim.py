#!/usr/bin/env python3
"""Verilator/iverilog co-sim adjudication for a Pavona keymgr module.
...
Reuses the per-module elaboration produced by run_keymgr_equiv.sh
(work/<mod>/slpp_all/surelog.uhdm) and the same keymgr_srcs.py closure.

Usage: keymgr_cosim.py <module> [cycles] [seed]
"""
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pavona_keymgr_equiv
...
INCS   = [f"{HERE}/rtl/keymgr", f"{HERE}/rtl/pkg", f"{KMACD}/rtl/kmac", f"{ACCD}/rtl/pkg",
...
# run_keymgr_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
if not os.path.isdir(WORK) or not os.path.exists(f"{WORK}/slpp_all/surelog.uhdm"):
    print(f"{mod}: NO_RUN (no elaboration; run run_keymgr_equiv.sh first)")
    sys.exit(2)
os.chdir(WORK)
...
# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/keymgr_srcs.py", mod]).stdout.split()
if os.path.exists(WRAPPER):
    SR.append(WRAPPER)
