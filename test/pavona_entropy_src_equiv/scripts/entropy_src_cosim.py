#!/usr/bin/env python3
"""Verilator/iverilog co-sim adjudication for a Pavona entropy_src module.
...
Reuses the per-module elaboration produced by run_entropy_src_equiv.sh
(work/<mod>/slpp_all/surelog.uhdm) and the same entropy_src_srcs.py closure.

Usage: entropy_src_cosim.py <module> [cycles] [seed]
"""
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pavona_entropy_src_equiv
...
INCS   = [f"{HERE}/rtl/entropy_src", f"{KMACD}/rtl/kmac", f"{ACCD}/rtl/pkg",
...
# run_entropy_src_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
if not os.path.isdir(WORK) or not os.path.exists(f"{WORK}/slpp_all/surelog.uhdm"):
    print(f"{mod}: NO_RUN (no elaboration; run run_entropy_src_equiv.sh first)")
    sys.exit(2)
os.chdir(WORK)
...
# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/entropy_src_srcs.py", mod]).stdout.split()
if os.path.exists(WRAPPER):
    SR.append(WRAPPER)
...
DIRECTED = {   # per-module overrides: {"input_name": "<verilog expr of i>"}
    # noise source always valid: a 96-sample bypass window fills every ~100 cycles
    "entropy_src_drv": {"rng_valid_i": "1'b1"},
}
