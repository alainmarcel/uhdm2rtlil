#!/usr/bin/env bash
# Per-module AES (Pavona OpenTitan AES-128/192/256 accelerator, masked DOM
# S-box by default) equivalence: read_uhdm vs read_slang, SAT-mitered.  Sources are
# the per-module dependency closure (scripts/aes_srcs.py), which pulls the ACC
# RTL plus the shared prim library / base packages vendored for the TL-UL
# campaign (../pavona_tlul_equiv/rtl/{prim,tlul,pkg}).
#   ./run_aes_equiv.sh [module...]      # default: every manifest module
set -uo pipefail
...
INCS=(-I"$HERE/rtl/aes" -I"$TLUL/rtl/prim" -I"$TLUL/rtl/tlul" -I"$TLUL/rtl/pkg" -I"$ACC/rtl/pkg")
...
run_one() {
  ...
  local SR; SR=$(python3 "$HERE/scripts/aes_srcs.py" "$m")
  local top="$m"
...
MODS=(); if [ $# -gt 0 ]; then
  ...
  # there (an explicit `./run_aes_equiv.sh aes_core` used to run with the
  # 300 s default and report the manifest's SAT-hard rows as unexpected).
  for a in "$@"; do
    e=$(awk -v m="$a" '$1==m && $1!~/^#/ {print $1":"$2":"$3":"$4; exit}' "$HERE/aes_modules.txt")
    MODS+=("${e:-$a}")
  done
else
  while read -r n s t w _; do [ -n "${n:-}" ]&&[[ ! "$n" =~ ^# ]]&&MODS+=("$n:$s:$t:$w"); done < "$HERE/aes_modules.txt"
fi
...
echo "AES equivalence: $proven/$total proven, $reg unexpected"
