#!/usr/bin/env python3
"""Verilator/iverilog co-sim adjudication for a Pavona AES module.
...
Reuses the per-module elaboration produced by run_aes_equiv.sh
(work/<mod>/slpp_all/surelog.uhdm) and the same aes_srcs.py closure.

Usage: aes_cosim.py <module> [cycles] [seed]
"""
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pavona_aes_equiv
...
INCS   = [f"{HERE}/rtl/aes", f"{ACCD}/rtl/pkg",
...
# run_aes_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
if not os.path.isdir(WORK) or not os.path.exists(f"{WORK}/slpp_all/surelog.uhdm"):
    print(f"{mod}: NO_RUN (no elaboration; run run_aes_equiv.sh first)")
    sys.exit(2)
os.chdir(WORK)
...
# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/aes_srcs.py", mod]).stdout.split()
if os.path.exists(WRAPPER):
    SR.append(WRAPPER)
