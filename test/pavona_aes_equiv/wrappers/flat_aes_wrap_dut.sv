// aes_wrap (OpenTitan's SCA/FPGA wrapper: a TL-UL master FSM that configures
// the core, loads key/IV/plaintext through the register file and reads the
// ciphertext back) with ONE fix: the vendored aes_wrap drives the request
// struct's a_user.data_intg TWICE — tlul_cmd_intg_gen's whole-struct tl_o
// (data_intg passed through as '0) and the wrapper's own prim_secded encoder
// output concat.  Both read_uhdm and read_slang netlists carry the 7-bit
// conflict (`check`: 7 problems), and Verilator resolves the two continuous
// drivers by statement order — the RTL / read_slang sim happened to keep the
// encoder value, the read_uhdm netlist the zero, so the register file rejected
// every write and the "divergence" was the wrapper's multi-driver, not the
// frontend.  Here tlul_cmd_intg_gen generates data_intg itself
// (EnableDataIntgGen=1) and the extra encoder is dropped.  Wrapper-only top
// (aes_wrap_dut_flat): aes_srcs.py seeds the closure from this file.
module aes_wrap_dut_flat
  import aes_pkg::*;
#(
  ...
  tlul_cmd_intg_gen #(.EnableDataIntgGen(1'b1)) tlul_cmd_intg_gen (
    .tl_i(h2d),
    .tl_o(h2d_intg)
  );
