#!/usr/bin/env python3
"""Verilator/iverilog co-sim adjudication for a Pavona small-peripheral (set 2) module.
...
Reuses the per-module elaboration produced by run_periph2_equiv.sh
(work/<mod>/slpp_all/surelog.uhdm) and the same periph2_srcs.py closure.

Usage: periph2_cosim.py <module> [cycles] [seed]
"""
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # pavona_periph2_equiv
...
KMACD  = os.path.normpath(os.path.join(HERE, '..', 'pavona_kmac_equiv'))
INCS   = [f"{HERE}/rtl/sysrst_ctrl", f"{HERE}/rtl/i2c", f"{HERE}/rtl/spi_host", f"{HERE}/rtl/adc_ctrl", f"{HERE}/rtl/pkg",
          f"{TLUL}/rtl/prim", f"{TLUL}/rtl/tlul", f"{TLUL}/rtl/pkg", f"{ACCD}/rtl/pkg"]
...
# run_periph2_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
if not os.path.isdir(WORK) or not os.path.exists(f"{WORK}/slpp_all/surelog.uhdm"):
    print(f"{mod}: NO_RUN (no elaboration; run run_periph2_equiv.sh first)")
    sys.exit(2)
os.chdir(WORK)

def sh(cmd, **kw):
    # errors="replace": a Verilator build can print non-UTF-8 bytes (keymgr_ctrl),
    # which used to abort the harness with UnicodeDecodeError.
    return subprocess.run(cmd, capture_output=True, text=True, errors="replace", **kw)

# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/periph2_srcs.py", mod]).stdout.split()
# spi_host_fsm / spi_host_core use `ASSERT without including prim_assert.sv
# (the IP relies on one compilation unit): prim_assert.sv first, slang --single-unit.
SR = [f"{TLUL}/rtl/prim/prim_assert.sv"] + SR
if os.path.exists(WRAPPER):
    SR.append(WRAPPER)
...
design -reset
read_slang --ignore-assertions -DSYNTHESIS --single-unit {inc_ys} {' '.join(SR)} --top {TOP}
hierarchy -check -top {TOP}
flatten; proc; memory; opt -fast; setundef -undriven -zero
delete t:$check t:$assert t:$assume t:$print
simplemap t:$bwmux
...
DIRECTED = {   # per-module overrides: {"input_name": "<verilog expr of i>"}
    # sysrst_ctrl debounce / detect blocks: random inputs never hold a level
    # for the configured debounce + detect time (and the 16/32-bit timer
    # configs are random), so every output stayed 0 for 300 cycles.  Slow
    # square waves on the key inputs, small timers, all enables set.
    "sysrst_ctrl_detect": {"cfg_enable_i": "1'b1", "cfg_debounce_timer_i": "16'd3",
                           "cfg_detect_timer_i": "32'd4", "trigger_i": "((i/24)%2)"},
    "sysrst_ctrl_combo": {"pwrb_int_i": "((i/30)%2)", "key0_int_i": "((i/30)%2)",
                          "key1_int_i": "((i/45)%2)", "key2_int_i": "((i/45)%2)",
                          "ac_present_int_i": "((i/60)%2)", "ec_rst_l_int_i": "1'b1",
                          "ec_rst_ctl_i": "16'd2", "key_intr_debounce_ctl_i": "16'd2",
                          # {ac_present, pwrb, key2, key1, key0} per combo (element 3
                          # first): combo0/1 pwrb+key0 with no precondition, combo2/3
                          # key1+key2 preconditioned on ac_present held low — the combo
                          # detector is EdgeToHigh on "all selected keys low", so a
                          # precondition on the SAME keys never sees an edge.
                          "com_pre_sel_ctl_i": "{5'b10000, 5'b10000, 5'b00000, 5'b00000}",
                          "com_sel_ctl_i": "{5'b00110, 5'b00110, 5'b01001, 5'b01001}",
                          "com_pre_det_ctl_i": "{4{32'd2}}", "com_det_ctl_i": "{4{32'd3}}",
                          "com_out_ctl_i": "{4{4'b1111}}"},
    "sysrst_ctrl_keyintr": {"key_intr_ctl_i": "14'h3fff", "key_intr_debounce_ctl_i": "16'd2",
                            "pwrb_int_i": "((i/20)%2)", "key0_int_i": "((i/26)%2)",
                            "key1_int_i": "((i/32)%2)", "key2_int_i": "((i/38)%2)",
                            "ac_present_int_i": "((i/44)%2)", "ec_rst_l_int_i": "((i/50)%2)",
                            "flash_wp_l_int_i": "((i/56)%2)"},
    "sysrst_ctrl_ulp": {"ulp_ctl_i": "1'b1", "ulp_ac_debounce_ctl_i": "16'd2",
                        "ulp_lid_debounce_ctl_i": "16'd2", "ulp_pwrb_debounce_ctl_i": "16'd2",
                        "lid_open_int_i": "((i/30)%2)", "ac_present_int_i": "((i/40)%2)",
                        "pwrb_int_i": "((i/50)%2)"},
}
...
TRACE_INIT = '    $dumpfile("cs.vcd"); $dumpvars(0, tb);' if os.environ.get("CS_TRACE") else ""
...
def run_verilator():
    r = sh(["verilator", "--binary", "-j", "0", "-Wno-lint", "-Wno-style",
            ...
            "-f", FLIST] + (["--trace"] if os.environ.get("CS_TRACE") else []) +
