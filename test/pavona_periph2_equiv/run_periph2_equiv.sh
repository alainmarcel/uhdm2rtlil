#!/usr/bin/env bash
# Per-module small peripherals set 2 (Pavona OpenTitan sysrst_ctrl, i2c,
# spi_host, adc_ctrl) equivalence: read_uhdm vs read_slang, SAT-mitered.  Sources are
# the per-module dependency closure (scripts/periph2_srcs.py), which pulls the ACC
# RTL plus the shared prim library / base packages vendored for the TL-UL
# campaign (../pavona_tlul_equiv/rtl/{prim,tlul,pkg}).
#   ./run_periph2_equiv.sh [module...]      # default: every manifest module
set -uo pipefail
...
INCS=(-I"$HERE/rtl/sysrst_ctrl" -I"$HERE/rtl/i2c" -I"$HERE/rtl/spi_host" -I"$HERE/rtl/adc_ctrl" -I"$HERE/rtl/pkg" -I"$TLUL/rtl/prim" -I"$TLUL/rtl/tlul" -I"$TLUL/rtl/pkg" -I"$ACC/rtl/pkg")
...
run_one() {
  ...
  local SR; SR=$(python3 "$HERE/scripts/periph2_srcs.py" "$m")
  local top="$m"
...
read_slang --ignore-assertions -DSYNTHESIS --single-unit ${SINCS[@]} $SR --top $top
hierarchy -check -top $top
flatten; proc; opt; memory; async2sync; delete t:\$check t:\$assert t:\$assume t:\$print
...
MODS=(); if [ $# -gt 0 ]; then
  ...
  # there (an explicit `./run_periph2_equiv.sh aes_core` used to run with the
  # 300 s default and report the manifest's SAT-hard rows as unexpected).
  for a in "$@"; do
    e=$(awk -v m="$a" '$1==m && $1!~/^#/ {print $1":"$2":"$3":"$4; exit}' "$HERE/periph2_modules.txt")
    MODS+=("${e:-$a}")
  done
else
  while read -r n s t w _; do [ -n "${n:-}" ]&&[[ ! "$n" =~ ^# ]]&&MODS+=("$n:$s:$t:$w"); done < "$HERE/periph2_modules.txt"
fi
...
echo "PERIPH2 equivalence: $proven/$total proven, $reg unexpected"
