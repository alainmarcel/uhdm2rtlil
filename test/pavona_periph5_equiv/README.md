...
- **31 manifest rows: 31 proven**
  (`./run_periph5_equiv.sh`, 2026-09-14): usbdev 14 / 14 (12 sub-modules,
  the `usbdev` top with its packet-buffer RAM bounded to 64 words, and the
  self-driving `usbdev_selftest` top, see below); spi_device 17 / 17
  (incl. `spi_device_reg_top` through its flat wrapper, and `spid_dpram` +
  the `spi_device` top through the global-clock flow for their dual-clock
  RAM, see below — seq=8 global steps, 282 s for the top).
- **Co-sim (300 cycles, seed 1; `usbdev_selftest` 3200): 31 / 31
  `NO_DIVERGENCE`** except `spid_status`, where both netlists differ
  identically from the RTL (an RTL-side artefact, see below).  Real activity
  on the USB engines (usb_fs_nb_in_pe `in_xact_start_ep_o` 255), the SPI
  command / read / status / upload engines, both tops' `tl_o` and
  spi_device's `passthrough_o`.  The random `usbdev` top co-sim never enables
  the device (pull-up, USB data lines and interrupts stay still), so the
  USB datapath is covered by `usbdev_selftest` instead: 245 device transmit
  edges, 279 / 270 D+ / D− changes, 5 interrupt changes, 13 TL reads.
...
- **usbdev top (was SAT-hard)** — the 512x32 packet-buffer RAM mapped to
  FFs is what the solver could not close: no verdict in 1800 s at seq=4.
  `wrappers/memsize_usbdev.txt` bounds that `$mem_v2` with `setparam SIZE`
  on both sides before `memory_map` (the same bounded-address abstraction as
  the dual-clock rows, now available to single-clock rows too): 16 words
  48 s, 64 words 135 s (the manifest bound), 128 words 285 s, 256 words
  873 s; 16 words at seq=8 525 s.  The full 512 words at seq=4 and 64
  words at seq=8 still give no verdict in 1800 s.
- **usbdev_selftest** — `wrappers/flat_usbdev_selftest.sv`, generated by
  `scripts/gen_usbdev_selftest.py`: a clock/reset-only top where a TL-UL
  master (through `tlul_cmd_intg_gen`) and a USB full-speed host model run a
  fixed program.  Enable the pull-up, bus reset, write and read back the
  packet buffer through the SRAM window, set up endpoint 0, host SETUP +
  DATA0 (device ACKs, RX FIFO entry: buffer 1, 8 bytes, setup), arm
  `configin_0` and host IN (device transmits the 5-byte buffer, host ACKs,
  `pkt_sent` and `in_sent` set), host OUT + DATA1 (device ACKs, buffer 2,
  3 bytes), then read back interrupts, status, both RX FIFO entries and the
  received payloads, and clear the interrupts.  The host packets are NRZI
  encoded, bit-stuffed and CRC-5/CRC-16'd in the generator and replayed as a
  J/K/SE0 symbol ROM at 4 clk_i cycles per bit.  The co-sim script gained a
  per-module minimum length (the program ends after ~2750 cycles) and now
  builds the instance port list from non-empty groups (a top with no data
  inputs produced an empty positional connection).
