...
Reuses the per-module elaboration produced by run_periph5_equiv.sh
(work/<mod>/slpp_all/surelog.uhdm) and the same periph5_srcs.py closure.

Usage: periph5_cosim.py <module> [cycles] [seed]
"""
...
INCS   = [f"{HERE}/rtl/usbdev", f"{HERE}/rtl/spi_device", f"{HERE}/rtl/pkg", f"{KMACD}/rtl/kmac", f"{KEYMGRD}/rtl/keymgr",
...
# run_periph5_equiv.sh); the work dir's UHDM has that wrapper as its top, so
...
if not os.path.isdir(WORK) or not os.path.exists(f"{WORK}/slpp_all/surelog.uhdm"):
    print(f"{mod}: NO_RUN (no elaboration; run run_periph5_equiv.sh first)")
    sys.exit(2)
os.chdir(WORK)
...
# ---------------------------------------------------------------- source list
SR = sh([sys.executable, f"{HERE}/scripts/periph5_srcs.py", mod]).stdout.split()
