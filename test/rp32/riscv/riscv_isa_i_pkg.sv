...
///////////////////////////////////////////////////////////////////////////////
  ...
  // NOTE (uhdm2rtlil import): made `packed` and `integer siz` -> `logic signed
  // [31:0] siz` so this decoder struct is a packed (bit-addressable) type.
  // r5p_* modules take `dec_t` as a PORT; synth flattens it, and Verilator can
  // only drive a flat netlist port into a struct port when the struct is packed
  // (an unpacked struct field like `integer` cannot be bit-mapped).  Same
  // intent as the upstream "use packed structures everywhere" note.
  typedef struct packed {
