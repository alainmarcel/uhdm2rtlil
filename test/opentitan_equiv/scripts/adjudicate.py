#!/usr/bin/env python3
"""Verilator co-sim adjudication for an upstream OpenTitan module.

The opentitan sweep's formal column mitres read_uhdm vs read_slang; it says the
two frontends agree or differ but cannot say whether they match the *behavioural
RTL*.  This runs three instances under identical random stimulus:

    the behavioural RTL   (the reference — the same .sv)
...
per-module elaboration run_opentitan_equiv.sh already produced
(work/<mod>/slpp_all/surelog.uhdm) and the same source list.

DELIBERATELY a separate copy from pavona_equiv/scripts/adjudicate.py: pavona is
a hard FORK of OpenTitan whose RTL has diverged (of the 120 files the two trees
share, only 66 are byte-identical), so the two families share no sources, no
manifest and no harness -- a change made for one must not silently retune the
other.

OpenTitan relies on COMPILATION-UNIT scope for its assertion / flop macros:
otbn_kmac_if.sv uses `ASSERT and `PRIM_FLOP_SPARSE_FSM with no `include of its
own.  So prim_assert.sv / prim_flop_macros.sv lead the file list for every tool,
and read_slang additionally needs --single-unit (it compiles each file as its
own unit by default and otherwise reports "unknown macro").
...
HERE   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))   # opentitan_equiv
...
if not os.path.isdir(WORK) or not os.path.exists(f"{WORK}/slpp_all/surelog.uhdm"):
    print(f"{mod}: NO_RUN (no elaboration; run run_opentitan_equiv.sh first)")
    sys.exit(2)
os.chdir(WORK)
...
# ---------------------------------------------------------------- source list
def srcs():
    """Mirror of run_opentitan_equiv.sh srcs() -- keep the two in step."""
    out = [f"{PRIM}/prim_assert.sv", f"{PRIM}/prim_flop_macros.sv"]
    for p in ("prim_util_pkg", "prim_secded_pkg", "prim_mubi_pkg",
              "prim_cipher_pkg", "prim_trivium_pkg", "prim_count_pkg",
              "prim_ram_1p_pkg", "prim_pkg", "prim_sparse_fsm_pkg",
              "prim_alert_pkg", "prim_esc_pkg", "prim_otp_pkg",
              "prim_subreg_pkg"):
        f = f"{PRIM}/{p}.sv"
        if os.path.exists(f): out.append(f)
    out.append(f"{PKG}/top_pkg.sv")
    for p in ("lc_ctrl_reg_pkg", "lc_ctrl_state_pkg", "lc_ctrl_pkg",
              "entropy_src_pkg", "csrng_reg_pkg", "csrng_pkg", "edn_pkg",
              "otp_ctrl_pkg", "sha3_pkg", "kmac_pkg", "keymgr_reg_pkg",
              "keymgr_pkg", "keymgr_dpe_pkg"):
        f = f"{PKG}/{p}.sv"
        if os.path.exists(f): out.append(f)
    out += sorted(glob.glob(f"{TLUL}/*_pkg.sv"))
    out += [f"{OTBN}/otbn_reg_pkg.sv", f"{OTBN}/otbn_pkg.sv"]
    for f in sorted(glob.glob(f"{PRIM}/prim_*.sv")):
        if not re.search(r"_pkg\.sv$|prim_assert\.sv$|prim_flop_macros\.sv$", f):
            out.append(f)
    for f in sorted(glob.glob(f"{TLUL}/*.sv")):
        if not re.search(r"_pkg\.sv$", f): out.append(f)
    for f in sorted(glob.glob(f"{OTBN}/otbn_*.sv")):
        if not re.search(r"_pkg\.sv$", f): out.append(f)
    return out
...
# plugin / the UHDM it reads.  A cached adj_gold.v silently kept adjudicating a
...
if _stale("adj_gold.v") or _stale("adj_gate.v"):
...
design -reset
read_slang --single-unit --ignore-assertions -DSYNTHESIS {' '.join(gparams)} -I {PRIM} -I {TLUL} -I {OTBN} -I {PKG} -f {FLIST} {slang_wrap} --top {TOP}
hierarchy -check -top {TOP}
flatten; proc; memory; opt -fast; setundef -undriven -zero
delete t:$check t:$assert t:$assume t:$print
simplemap t:$bwmux
...
# ACTIVITY: cycles in which any monitored RTL output CHANGED.  Without it a
# co-sim that never exercised the DUT reports NO_DIVERGENCE and reads exactly
# like a real pass (keccak_round's vacuous NO_DIVERGENCE).  Every other sweep
# family reports this; opentitan printed no ACTIVITY line at all, so its table
# showed "? active".
...
def run_iverilog():
    inc = [f"-I{PRIM}", f"-I{TLUL}", f"-I{OTBN}", f"-I{PKG}",
           f"-I{HERE}/wrappers"]
