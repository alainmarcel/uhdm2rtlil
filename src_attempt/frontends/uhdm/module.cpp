        ...
        if (uhdm_port->Typespec()) {
            ...
            if (typespec && typespec->UhdmType() == uhdminterface_typespec) {
                ...
                if (!interface_type.empty()) {
                    ...
                    // For interfaces, we don't use parameterized names
                    // The interface_type is always just the base interface name
                    w->attributes[RTLIL::escape_id("interface_type")] = RTLIL::Const("\\" + interface_type);
                }
...
void UhdmImporter::import_instance(const module_inst* uhdm_inst) {
    ...
    // For regular modules (not interfaces), don't create parameterized versions
    // Just use the base module name and set parameters on the cell
    ...
    // If the module has interface ports, we don't modify the module name
    ...
    if (!design->module(module_id)) {
        ...
        // Don't create parameterized versions - use base module directly
        // The parameters will be set on the cell instance
        if (!params.empty() && design->module(base_module_id)) {
            log("UHDM: Using base module %s with parameters\n", base_module_name.c_str());
        }
    }
    
    // Use base module name for cell type (not parameterized name)
    RTLIL::IdString cell_type = RTLIL::escape_id(base_module_name);
    log("UHDM: Creating cell '%s' of type '%s'\n", inst_name.c_str(), cell_type.c_str());
    RTLIL::Cell* cell = module->addCell(new_id(inst_name), cell_type);
    
    // Add source attribute to cell
    add_src_attribute(cell->attributes, uhdm_inst);
    
    // Add module_not_derived attribute for cells with parameters
    if (!params.empty()) {
