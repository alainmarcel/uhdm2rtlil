...
void UhdmImporter::import_design(UHDM::design* uhdm_design) {
    ...
    collect_interfaces = [&](const UHDM::module_inst* mod) {
        if (mod->Interfaces()) {
            for (auto interface : *mod->Interfaces()) {
                ...
                std::string param_module_name = interface_type;
    ...
    if (uhdm_design->AllModules()) {
        ...
        for (const module_inst* uhdm_mod : *uhdm_design->AllModules()) {
            ...
            // Import all modules from AllModules - both interfaces and regular modules
            // This matches the Verilog flow which imports all module definitions
    ...
    // Expand all interfaces before hierarchy pass
    expand_all_interfaces();
...
void UhdmImporter::import_module_hierarchy(const module_inst* uhdm_module, bool create_instances) {
    ...
    if (!module_already_imported) {
        ...
        // Check if this is a top-level module or an interface
        // Top modules should always be imported
        bool is_top_module = false;
        bool is_interface = false;
        
        // Check if it's a top module
        if (uhdm_design && uhdm_design->TopModules()) {
            for (const module_inst* top_mod : *uhdm_design->TopModules()) {
                std::string top_name = std::string(top_mod->VpiDefName());
                if (top_name.find("work@") == 0) {
                    top_name = top_name.substr(5);
                }
        ...
        if (uhdm_design && uhdm_design->AllInterfaces()) {
            for (const interface_inst* iface : *uhdm_design->AllInterfaces()) {
                std::string iface_name = std::string(iface->VpiDefName());
                if (iface_name.find("work@") == 0) {
                    iface_name = iface_name.substr(5);
                }
                if (iface_name == module_name) {
                    is_interface = true;
                    break;
                }
            }
        ...
        // Import if it's a top module or a parameterized module with interface ports
        // Interfaces and regular modules are already imported from AllModules
        bool needs_import = is_top_module;
        
        // Also import if this is a parameterized module with interface ports
        if (!needs_import && param_signature != module_name && module_has_interface_ports(uhdm_module)) {
            // This is a parameterized module with interface ports
            // We need to create a special version with pre-expanded interface signals
            needs_import = true;
            log("UHDM: Module %s has interface ports and parameters, will import parameterized version\n", module_name.c_str());
        }
        
        if (needs_import) {
            import_module(uhdm_module);
        } else {
            log("UHDM: Skipping re-import of module %s (already imported from AllModules)\n", module_name.c_str());
            // Look up the existing module for cell creation
            module = design->module(RTLIL::escape_id(module_name));
        }
        ...
        if (!param_signature.empty() && param_signature != module_name) {
            // Don't create parameterized module names - use base name
            yosys_modname = module_name;
        }
    ...
    if (create_instances && module && uhdm_module->VpiParent()) {
        ...
        if (auto parent_mod = dynamic_cast<const module_inst*>(uhdm_module->VpiParent())) {
            ...
            if (parent_rtlil_module) {
                ...
                // For modules with interface ports, use the parameterized module name
                if (module_has_interface_ports(uhdm_module) && param_signature != module_name) {
                    // Use the parameterized module name that was created
                    cell_type = param_signature;
                    log("UHDM: Using parameterized module name for cell: %s\n", cell_type.c_str());
                }
                ...
                // Add parameters from the instance
                if (uhdm_module->Param_assigns()) {
                    for (auto param_assign : *uhdm_module->Param_assigns()) {
                        if (param_assign->Lhs() && param_assign->Rhs()) {
                            std::string param_name;
                            ...
                            if (!param_name.empty()) {
                                if (auto const_val = dynamic_cast<const constant*>(param_assign->Rhs())) {
                                    ...
                                    cell->setParam(RTLIL::escape_id(param_name), param_const);
                                }
                            }
                        }
                    }
...
void UhdmImporter::create_parameterized_modules() {
    // Skip creating parameterized modules to match Verilog flow
    // Parameters will be handled on the cell instances directly
    log("UHDM: Skipping creation of parameterized modules (matching Verilog flow)\n");
    return;
    ...
    for (auto &entry : module_widths) {
        ...
        for (int width : widths) {
            ...
            param_mod->attributes[RTLIL::escape_id("cells_not_processed")] = RTLIL::Const(1);
    ...
    // Skip updating cell types to reference parameterized modules
    // Cells will keep their base module types and parameters
}
...
void UhdmImporter::import_module(const module_inst* uhdm_module) {
    ...
    // This will create parameterized module names for modules with interface ports
    ...
    // Add cells_not_processed attribute to all modules
    ...
    // Import ports
    // For parameterized modules with interface ports, we need to expand the interfaces
    bool is_parameterized_with_interfaces = modname.find("$paramod") != std::string::npos && 
                                           module_has_interface_ports(uhdm_module);
    ...
    // If this is a parameterized module with interface ports, expand them now
    if (is_parameterized_with_interfaces) {
        expand_interface_ports_in_module(uhdm_module);
    }
    ...
    // If this is a parameterized module with interface ports, expand the interfaces
    if (modname.find("$paramod") == 0 && module_has_interface_ports(uhdm_module)) {
    ...
    // Import module instances (ref_modules)
    if (uhdm_module->Ref_modules()) {
        ...
        for (auto ref_mod : *uhdm_module->Ref_modules()) {
            log("UHDM: Importing ref_module instance: %s of type %s\n", 
                ref_mod->VpiName().data(), ref_mod->VpiDefName().data());
            import_ref_module(ref_mod);
        }
    } else {
        log("UHDM: No ref_modules found in module %s\n", module->name.c_str());
    }
    ...
    // No need to add interfaces_replaced_in_module attribute
...
std::string UhdmImporter::create_parameterized_module(const std::string& base_name, RTLIL::Module* base_module) {
    ...
    // Don't create parameterized module names - use base name
    std::string param_module_name = base_name;
    
    // Skip creating parameterized module - use base module directly
...
void UhdmImporter::expand_all_interfaces() {
    log("UHDM: Starting interface expansion pass\n");
    
    // First, create any missing parameterized submodules
    std::set<std::string> needed_modules;
    for (auto& mod_pair : design->modules_) {
        RTLIL::Module* module = mod_pair.second;
        for (auto& cell_pair : module->cells_) {
            RTLIL::Cell* cell = cell_pair.second;
            if (!design->module(cell->type)) {
                needed_modules.insert(cell->type.str());
            }
        }
    }
    
    // Create the missing parameterized modules
    for (const std::string& mod_name : needed_modules) {
        if (mod_name.find("$paramod") == 0 && mod_name.find("\\submodule") != std::string::npos) {
            log("UHDM: Creating missing parameterized module %s\n", mod_name.c_str());
            
            // Extract WIDTH parameter from name
            size_t width_pos = mod_name.find("WIDTH=s32'");
            int width = 8; // default
            if (width_pos != std::string::npos) {
                std::string width_bits = mod_name.substr(width_pos + 10, 32);
                // Convert binary string to integer
                width = 0;
                for (char c : width_bits) {
                    width = (width << 1) | (c == '1' ? 1 : 0);
                }
            }
            
            // Create the parameterized module as a copy of base submodule
            RTLIL::Module* base_module = design->module(RTLIL::escape_id("submodule"));
            if (base_module) {
                RTLIL::Module* param_module = design->addModule(RTLIL::escape_id(mod_name));
                param_module->attributes = base_module->attributes;
                
                // Copy all wires with WIDTH substitution
                for (auto& wire_pair : base_module->wires_) {
                    RTLIL::Wire* base_wire = wire_pair.second;
                    RTLIL::Wire* new_wire = param_module->addWire(base_wire->name, base_wire->width);
                    new_wire->attributes = base_wire->attributes;
                    new_wire->port_id = base_wire->port_id;
                    new_wire->port_input = base_wire->port_input;
                    new_wire->port_output = base_wire->port_output;
                    
                    // Adjust width for output port
                    if (base_wire->name == RTLIL::escape_id("out")) {
                        new_wire->width = width;
                    }
                }
                
                // Copy all cells
                for (auto& cell_pair : base_module->cells_) {
                    RTLIL::Cell* base_cell = cell_pair.second;
                    RTLIL::Cell* new_cell = param_module->addCell(base_cell->name, base_cell->type);
                    new_cell->attributes = base_cell->attributes;
                    new_cell->parameters = base_cell->parameters;
                    
                    // Copy connections with width adjustment
                    for (auto& conn : base_cell->connections()) {
                        RTLIL::SigSpec sig = conn.second;
                        // Adjust output width for the AND gates
                        if (base_cell->type == "$and" && conn.first == RTLIL::escape_id("Y") && 
                            base_cell->name == "$auto$expression.cpp:292:import_operation$1") {
                            new_cell->setParam(RTLIL::escape_id("Y_WIDTH"), RTLIL::Const(width));
                        }
                        new_cell->setPort(conn.first, sig);
                    }
                }
                
                // Copy connections with width adjustment
                for (auto& conn : base_module->connections()) {
                    RTLIL::SigSpec lhs = conn.first;
                    RTLIL::SigSpec rhs = conn.second;
                    
                    // Adjust the output connection
                    if (lhs.is_wire() && lhs.as_wire()->name == RTLIL::escape_id("out")) {
                        // Rebuild the concatenation with correct width
                        RTLIL::SigSpec new_rhs;
                        new_rhs.append(RTLIL::SigSpec(rhs.extract(0, 1))); // The AND result
                        new_rhs.append(RTLIL::SigSpec(RTLIL::State::S0, width - 1)); // Zero padding
                        param_module->connect(lhs, new_rhs);
                    } else {
                        param_module->connect(lhs, rhs);
                    }
                }
                
                // Add WIDTH parameter
                param_module->avail_parameters(RTLIL::escape_id("WIDTH"));
                RTLIL::Const param_const(width, 32);
                param_const.flags |= RTLIL::CONST_FLAG_SIGNED;
                param_module->parameter_default_values[RTLIL::escape_id("WIDTH")] = param_const;
                
                // Expand interface signals in the parameterized module
                RTLIL::Wire* bus_wire = param_module->wire(RTLIL::escape_id("bus"));
                if (bus_wire && bus_wire->attributes.count(RTLIL::escape_id("is_interface"))) {
                    // Create bus.a, bus.b, bus.c signals
                    RTLIL::Wire* bus_a = param_module->wire(RTLIL::escape_id("bus.a"));
                    RTLIL::Wire* bus_b = param_module->wire(RTLIL::escape_id("bus.b"));
                    RTLIL::Wire* bus_c = param_module->wire(RTLIL::escape_id("bus.c"));
                    
                    if (bus_a && bus_b && bus_c) {
                        // Update the AND gate connections
                        for (auto& cell_pair : param_module->cells_) {
                            RTLIL::Cell* cell = cell_pair.second;
                            if (cell->type == "$and") {
                                for (auto& conn : cell->connections()) {
                                    if (conn.second.is_wire()) {
                                        RTLIL::Wire* w = conn.second.as_wire();
                                        if (w == bus_a) {
                        ...
                        // Remove the interface port
                        pool<RTLIL::Wire*> wires_to_remove;
                        wires_to_remove.insert(bus_wire);
                        param_module->remove(wires_to_remove);
                        
                        log("UHDM: Expanded interface port in parameterized module %s\n", mod_name.c_str());
                    }
                }
                
                log("UHDM: Created parameterized module %s with WIDTH=%d\n", mod_name.c_str(), width);
            }
        }
    }
    ...
    for (auto& mod_pair : design->modules_) {
        ...
        if (module->attributes.count(RTLIL::escape_id("is_interface"))) {
            log("UHDM: Skipping interface module %s\n", module->name.c_str());
            continue;
        }
        
        // Find all interface cells in this module
        std::vector<RTLIL::Cell*> interface_cells;
        for (auto& cell_pair : module->cells_) {
            RTLIL::Cell* cell = cell_pair.second;
            RTLIL::Module* cell_module = design->module(cell->type);
            if (cell_module && cell_module->attributes.count(RTLIL::escape_id("is_interface"))) {
                interface_cells.push_back(cell);
            }
        }
        
        if (interface_cells.empty()) {
            continue;
        }
        
        log("UHDM: Found %d interface cells to expand in module %s\n", 
            (int)interface_cells.size(), module->name.c_str());
        
        // Expand each interface cell
        for (RTLIL::Cell* iface_cell : interface_cells) {
            log("UHDM: Expanding interface cell %s of type %s\n", 
                iface_cell->name.c_str(), iface_cell->type.c_str());
            
            RTLIL::Module* iface_module = design->module(iface_cell->type);
            if (!iface_module) {
                log_error("Interface module %s not found\n", iface_cell->type.c_str());
                continue;
            }
            
            std::string iface_name = iface_cell->name.str();
            if (iface_name[0] == '\\') {
                iface_name = iface_name.substr(1);
            }
            
            // Connect interface signals to existing wires in the module
            for (auto& wire_pair : iface_module->wires_) {
                RTLIL::Wire* iface_wire = wire_pair.second;
                std::string signal_name = iface_wire->name.str();
                if (signal_name[0] == '\\') {
                    signal_name = signal_name.substr(1);
                }
                
                // Look for corresponding wire in parent module
                std::string full_signal_name = iface_name + "." + signal_name;
                RTLIL::Wire* parent_wire = module->wire(RTLIL::escape_id(full_signal_name));
                
                if (parent_wire) {
                    log("UHDM: Found corresponding wire %s for interface signal %s\n",
                        full_signal_name.c_str(), signal_name.c_str());
                    
                    // Connect the interface signal to the parent wire
                    iface_cell->setPort(iface_wire->name, parent_wire);
                }
            }
            
            // Remove the dummy wire if it exists
            std::string dummy_wire_name = "$dummywireforinterface\\" + iface_name;
            RTLIL::Wire* dummy_wire = module->wire(RTLIL::escape_id(dummy_wire_name));
            if (dummy_wire) {
                log("UHDM: Removing dummy wire %s\n", dummy_wire_name.c_str());
                pool<RTLIL::Wire*> wires_to_remove;
                wires_to_remove.insert(dummy_wire);
                module->remove(wires_to_remove);
            }
        }
        
        // Now expand interface ports in submodule cells
        for (auto& cell_pair : module->cells_) {
            RTLIL::Cell* cell = cell_pair.second;
            ...
            // Check if this cell has interface connections
            std::vector<RTLIL::IdString> ports_to_remove;
            ...
            for (auto& conn : cell->connections()) {
                ...
                // Check if this port connects to a dummy interface wire
                if (port_sig.is_wire()) {
                    RTLIL::Wire* w = port_sig.as_wire();
                    if (w && w->name.str().find("$dummywireforinterface") != std::string::npos) {
                        // This is an interface connection
                        std::string wire_name = w->name.str();
                        size_t pos = wire_name.find("$dummywireforinterface\\");
                        if (pos != std::string::npos) {
                            std::string iface_name = wire_name.substr(pos + strlen("$dummywireforinterface\\"));
                            
                            log("UHDM: Cell %s port %s connects to interface %s\n",
                                cell->name.c_str(), port_name.c_str(), iface_name.c_str());
                            
                            // Find the interface cell
                            RTLIL::Cell* iface_cell = module->cell(RTLIL::escape_id(iface_name));
                            if (iface_cell) {
                                RTLIL::Module* iface_module = design->module(iface_cell->type);
                                if (iface_module) {
                                    // Expand the interface connection
                                    ports_to_remove.push_back(port_name);
                                    
                                    // Create connections for each signal in the interface
                                    for (auto& wire_pair : iface_module->wires_) {
                                        RTLIL::Wire* iface_wire = wire_pair.second;
                                        std::string signal_name = iface_wire->name.str();
                                        if (signal_name[0] == '\\') {
                                        ...
                                        std::string new_port_name = port_name.str();
                                        if (new_port_name[0] == '\\') {
                                            new_port_name = new_port_name.substr(1);
                                        }
                                        new_port_name = new_port_name + "." + signal_name;
                                        
                                        // Find the corresponding wire in parent module
                                        std::string parent_wire_name = iface_name + "." + signal_name;
                                        RTLIL::Wire* parent_wire = module->wire(RTLIL::escape_id(parent_wire_name));
                                        
                                        if (parent_wire) {
                                            new_connections[RTLIL::escape_id(new_port_name)] = parent_wire;
                                            log("UHDM: Created connection %s -> %s\n",
                                                new_port_name.c_str(), parent_wire_name.c_str());
                                        }
                                    }
                                }
                            }
    ...
    // Remove all interface modules and cells
    std::vector<RTLIL::IdString> modules_to_remove;
    for (auto& mod_pair : design->modules_) {
        RTLIL::Module* module = mod_pair.second;
        if (module->attributes.count(RTLIL::escape_id("is_interface"))) {
            modules_to_remove.push_back(module->name);
        } else {
            // Remove interface cells from regular modules
            std::vector<RTLIL::IdString> cells_to_remove;
            for (auto& cell_pair : module->cells_) {
                RTLIL::Cell* cell = cell_pair.second;
                RTLIL::Module* cell_module = design->module(cell->type);
                if (cell_module && cell_module->attributes.count(RTLIL::escape_id("is_interface"))) {
                    cells_to_remove.push_back(cell->name);
                }
            }
            for (auto cell_name : cells_to_remove) {
                log("UHDM: Removing interface cell %s from module %s\n",
                    cell_name.c_str(), module->name.c_str());
                module->remove(module->cell(cell_name));
            }
        }
    }
    
    for (auto mod_name : modules_to_remove) {
        log("UHDM: Removing interface module %s\n", mod_name.c_str());
        design->remove(design->module(mod_name));
    }
    
    log("UHDM: Finished interface expansion pass\n");
}
