...
/*
 * File:   sequence_expr_multiclock_group.cpp
 * Author:
 *
...
bool sequence_expr_multiclock_groupGroupCompliant(const any* item) {
  ...
  if ((uhdmtype != uhdmarray_expr) &&
    ...
    item->GetSerializer()->GetErrorHandler()(ErrorType::UHDM_WRONG_OBJECT_TYPE, "Internal Error: adding wrong object type (" + UhdmName(uhdmtype) + ") in a sequence_expr_multiclock_group group!", item, nullptr);
    return false;
  }
  return true;
}
...
bool sequence_expr_multiclock_groupGroupCompliant(const VectorOfany* vec) {
  if (vec != nullptr) {
    for (auto item : *vec) {
      if (!sequence_expr_multiclock_groupGroupCompliant(item)) {
        return false;
      }
    }
  }
