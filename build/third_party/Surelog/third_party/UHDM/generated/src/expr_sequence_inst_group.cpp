...
/*
 * File:   expr_sequence_inst_group.cpp
 * Author:
 *
...
bool expr_sequence_inst_groupGroupCompliant(const any* item) {
  ...
  if ((uhdmtype != uhdmarray_expr) &&
    ...
    item->GetSerializer()->GetErrorHandler()(ErrorType::UHDM_WRONG_OBJECT_TYPE, "Internal Error: adding wrong object type (" + UhdmName(uhdmtype) + ") in a expr_sequence_inst_group group!", item, nullptr);
    return false;
  }
  return true;
}
...
bool expr_sequence_inst_groupGroupCompliant(const VectorOfany* vec) {
  if (vec != nullptr) {
    for (auto item : *vec) {
      if (!expr_sequence_inst_groupGroupCompliant(item)) {
        return false;
      }
    }
  }
