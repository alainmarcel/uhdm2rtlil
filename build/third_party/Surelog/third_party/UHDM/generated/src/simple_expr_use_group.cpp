...
/*
 * File:   simple_expr_use_group.cpp
 * Author:
 *
...
bool simple_expr_use_groupGroupCompliant(const any* item) {
  ...
  if ((uhdmtype != uhdmalways) &&
      ...
      (uhdmtype != uhdmpath_term) &&
      (uhdmtype != uhdmport) &&
      (uhdmtype != uhdmport_bit) &&
      (uhdmtype != uhdmports) &&
      (uhdmtype != uhdmprim_term) &&
    ...
    item->GetSerializer()->GetErrorHandler()(ErrorType::UHDM_WRONG_OBJECT_TYPE, "Internal Error: adding wrong object type (" + UhdmName(uhdmtype) + ") in a simple_expr_use_group group!", item, nullptr);
    return false;
  }
  return true;
}
...
bool simple_expr_use_groupGroupCompliant(const VectorOfany* vec) {
  if (vec != nullptr) {
    for (auto item : *vec) {
      if (!simple_expr_use_groupGroupCompliant(item)) {
        return false;
      }
    }
  }
