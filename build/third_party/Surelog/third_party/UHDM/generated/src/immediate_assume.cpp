...
namespace UHDM {
const BaseClass* immediate_assume::GetByVpiName(std::string_view name) const {
...
std::tuple<const BaseClass*, UHDM_OBJECT_TYPE, const std::vector<const BaseClass*>*> immediate_assume::GetByVpiType(int32_t type) const {
  switch (type) {
...
immediate_assume::vpi_property_value_t immediate_assume::GetVpiPropertyValue(int32_t property) const {
  switch (property) {
...
void immediate_assume::DeepCopy(immediate_assume* clone, BaseClass* parent, CloneContext* context) const {
  [[maybe_unused]] ElaboratorContext* const elaboratorContext = clonecontext_cast<ElaboratorContext*>(context);
  elaboratorContext->m_elaborator.enterImmediate_assume(clone, nullptr);
  ...
  elaboratorContext->m_elaborator.leaveImmediate_assume(clone, nullptr);
}

immediate_assume* immediate_assume::DeepClone(BaseClass* parent, CloneContext* context) const {
  immediate_assume* const clone = context->m_serializer->MakeImmediate_assume();
...
int32_t immediate_assume::Compare(const BaseClass *other, CompareContext* context) const {
  int32_t r = 0;
