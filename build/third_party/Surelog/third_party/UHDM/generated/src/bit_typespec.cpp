...
namespace UHDM {
const BaseClass* bit_typespec::GetByVpiName(std::string_view name) const {
  if ((bit_typespec_ != nullptr) && (bit_typespec_->VpiName().compare(name) == 0)) {
    return bit_typespec_;
  }
  if (ranges_ != nullptr) {
...
std::tuple<const BaseClass*, UHDM_OBJECT_TYPE, const std::vector<const BaseClass*>*> bit_typespec::GetByVpiType(int32_t type) const {
  switch (type) {
    case vpiElemTypespec: return std::make_tuple(bit_typespec_, static_cast<UHDM_OBJECT_TYPE>(0), nullptr);
...
bit_typespec::vpi_property_value_t bit_typespec::GetVpiPropertyValue(int32_t property) const {
  switch (property) {
...
void bit_typespec::DeepCopy(bit_typespec* clone, BaseClass* parent, CloneContext* context) const {
  [[maybe_unused]] ElaboratorContext* const elaboratorContext = clonecontext_cast<ElaboratorContext*>(context);
  elaboratorContext->m_elaborator.enterBit_typespec(clone, nullptr);
  basetype_t::DeepCopy(clone, parent, context);
  if (auto obj = Bit_typespec()) clone->Bit_typespec(obj->DeepClone(clone, context));
  if (auto vec = Ranges()) {
  ...
  elaboratorContext->m_elaborator.leaveBit_typespec(clone, nullptr);
}

bit_typespec* bit_typespec::DeepClone(BaseClass* parent, CloneContext* context) const {
  bit_typespec* const clone = context->m_serializer->MakeBit_typespec();
...
int32_t bit_typespec::Compare(const BaseClass *other, CompareContext* context) const {
  int32_t r = 0;
  ...
  if ((r = SafeCompare(lhs->Bit_typespec(), rhs->Bit_typespec(), context)) != 0) return r;
