...
bool pattern_expr_groupGroupCompliant(const any* item) {
  ...
  if ((uhdmtype != uhdmany_pattern) &&
    ...
    item->GetSerializer()->GetErrorHandler()(ErrorType::UHDM_WRONG_OBJECT_TYPE, "Internal Error: adding wrong object type (" + UhdmName(uhdmtype) + ") in a pattern_expr_group group!", item, nullptr);
    return false;
  }
  return true;
}
...
bool pattern_expr_groupGroupCompliant(const VectorOfany* vec) {
  if (vec != nullptr) {
    for (auto item : *vec) {
      if (!pattern_expr_groupGroupCompliant(item)) {
        return false;
      }
    }
  }
