...
namespace UHDM {
std::string_view atomic_stmt::VpiName() const {
  return vpiName_ ? serializer_->GetSymbol(vpiName_) : kEmpty;
}

bool atomic_stmt::VpiName(std::string_view data) {
  vpiName_ = serializer_->MakeSymbol(data);
  return true;
}

const BaseClass* atomic_stmt::GetByVpiName(std::string_view name) const {
...
std::tuple<const BaseClass*, UHDM_OBJECT_TYPE, const std::vector<const BaseClass*>*> atomic_stmt::GetByVpiType(int32_t type) const {
  switch (type) {
...
atomic_stmt::vpi_property_value_t atomic_stmt::GetVpiPropertyValue(int32_t property) const {
  switch (property) {
    case vpiName: {
...
void atomic_stmt::DeepCopy(atomic_stmt* clone, BaseClass* parent, CloneContext* context) const {
...
int32_t atomic_stmt::Compare(const BaseClass *other, CompareContext* context) const {
  int32_t r = 0;
