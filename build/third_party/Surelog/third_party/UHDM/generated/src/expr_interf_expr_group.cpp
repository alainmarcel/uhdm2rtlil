...
/*
 * File:   expr_interf_expr_group.cpp
 * Author:
 *
...
bool expr_interf_expr_groupGroupCompliant(const any* item) {
  ...
  if ((uhdmtype != uhdmany_pattern) &&
    ...
    item->GetSerializer()->GetErrorHandler()(ErrorType::UHDM_WRONG_OBJECT_TYPE, "Internal Error: adding wrong object type (" + UhdmName(uhdmtype) + ") in a expr_interf_expr_group group!", item, nullptr);
    return false;
  }
  return true;
}
...
bool expr_interf_expr_groupGroupCompliant(const VectorOfany* vec) {
  if (vec != nullptr) {
    for (auto item : *vec) {
      if (!expr_interf_expr_groupGroupCompliant(item)) {
        return false;
      }
    }
  }
