...
/*
 * File:   named_event_sequence_expr_group.cpp
 * Author:
 *
...
bool named_event_sequence_expr_groupGroupCompliant(const any* item) {
  ...
  if ((uhdmtype != uhdmarray_expr) &&
    ...
    item->GetSerializer()->GetErrorHandler()(ErrorType::UHDM_WRONG_OBJECT_TYPE, "Internal Error: adding wrong object type (" + UhdmName(uhdmtype) + ") in a named_event_sequence_expr_group group!", item, nullptr);
    return false;
  }
  return true;
}
...
bool named_event_sequence_expr_groupGroupCompliant(const VectorOfany* vec) {
  if (vec != nullptr) {
    for (auto item : *vec) {
      if (!named_event_sequence_expr_groupGroupCompliant(item)) {
        return false;
      }
    }
  }
