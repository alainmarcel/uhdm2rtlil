...
namespace UHDM {
std::string_view cont_assign_bit::VpiValue() const {
  return vpiValue_ ? serializer_->GetSymbol(vpiValue_) : kEmpty;
}

bool cont_assign_bit::VpiValue(std::string_view data) {
  vpiValue_ = serializer_->MakeSymbol(data);
  return true;
}

const BaseClass* cont_assign_bit::GetByVpiName(std::string_view name) const {
...
std::tuple<const BaseClass*, UHDM_OBJECT_TYPE, const std::vector<const BaseClass*>*> cont_assign_bit::GetByVpiType(int32_t type) const {
  switch (type) {
...
cont_assign_bit::vpi_property_value_t cont_assign_bit::GetVpiPropertyValue(int32_t property) const {
  switch (property) {
    case vpiNetDeclAssign: return vpi_property_value_t(VpiNetDeclAssign());
    case vpiOffset: return vpi_property_value_t(VpiOffset());
...
void cont_assign_bit::DeepCopy(cont_assign_bit* clone, BaseClass* parent, CloneContext* context) const {
  [[maybe_unused]] ElaboratorContext* const elaboratorContext = clonecontext_cast<ElaboratorContext*>(context);
  elaboratorContext->m_elaborator.enterCont_assign_bit(clone, nullptr);
  ...
  elaboratorContext->m_elaborator.leaveCont_assign_bit(clone, nullptr);
}

cont_assign_bit* cont_assign_bit::DeepClone(BaseClass* parent, CloneContext* context) const {
  cont_assign_bit* const clone = context->m_serializer->MakeCont_assign_bit();
...
int32_t cont_assign_bit::Compare(const BaseClass *other, CompareContext* context) const {
  int32_t r = 0;
  ...
  if ((r = lhs->VpiOffset() - rhs->VpiOffset()) != 0) {
