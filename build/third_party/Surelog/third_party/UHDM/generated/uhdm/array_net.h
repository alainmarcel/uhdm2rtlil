/*
 Do not modify, auto-generated by classes.py

 Copyright 2019 Alain Dargelas

 Licensed under the Apache License, Version 2.0 (the "License");
 you may not use this file except in compliance with the License.
 You may obtain a copy of the License at

 http://www.apache.org/licenses/LICENSE-2.0

 Unless required by applicable law or agreed to in writing, software
 distributed under the License is distributed on an "AS IS" BASIS,
 WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 See the License for the specific language governing permissions and
 limitations under the License.
 */

/*
 * File:   array_net.h
 * Author:
 *
 * Created on December 14, 2019, 10:03 PM
 */

#ifndef UHDM_ARRAY_NET_H
#define UHDM_ARRAY_NET_H

#include <uhdm/sv_vpi_user.h>
#include <uhdm/uhdm_vpi_user.h>

#include <uhdm/containers.h>
#include <uhdm/nets.h>




namespace UHDM {


class array_net final : public nets {
  UHDM_IMPLEMENT_RTTI(array_net, nets)
public:
  // Implicit constructor used to initialize all members,
  // comment: array_net();
  virtual ~array_net() final = default;

  VectorOfrange* Ranges() const { return ranges_; }
  bool Ranges(VectorOfrange* data) {
    ranges_ = data;
    return true;
  }

  VectorOfnet* Nets() const { return nets_; }
  bool Nets(VectorOfnet* data) {
    nets_ = data;
    return true;
  }

  VectorOfattribute* Attributes() const { return attributes_; }
  bool Attributes(VectorOfattribute* data) {
    attributes_ = data;
    return true;
  }

  virtual uint32_t VpiType() const final { return vpiArrayNet; }

  virtual array_net* DeepClone(BaseClass* parent, CloneContext* context) const override;

  virtual const BaseClass* GetByVpiName(std::string_view name) const override;

  virtual std::tuple<const BaseClass*, UHDM_OBJECT_TYPE, const std::vector<const BaseClass*>*> GetByVpiType(int32_t type) const override;

  virtual vpi_property_value_t GetVpiPropertyValue(int32_t property) const override;

  virtual int32_t Compare(const BaseClass* other, CompareContext* context) const override;

  virtual  UHDM_OBJECT_TYPE UhdmType() const final { return UHDM_OBJECT_TYPE::uhdmarray_net; }

protected:
  void DeepCopy(array_net* clone, BaseClass* parent, CloneContext* context) const;

private:
  VectorOfrange* ranges_ = nullptr;

  VectorOfnet* nets_ = nullptr;

  VectorOfattribute* attributes_ = nullptr;
};


typedef FactoryT<array_net> array_netFactory;


typedef FactoryT<std::vector<array_net *>> VectorOfarray_netFactory;

}  // namespace UHDM

#endif
