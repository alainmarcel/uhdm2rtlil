...
public:
  ...
  any* Property_expr()  { return property_expr_; }
  const any* Property_expr() const { return property_expr_; }
  template <typename T>
  T* Property_expr() {
    return (property_expr_ == nullptr) ? nullptr : any_cast<T*>(property_expr_);
  }
  template <typename T>
  const T* Property_expr() const {
    return (property_expr_ == nullptr) ? nullptr : any_cast<const T*>(property_expr_);
  }
  bool Property_expr(any* data) {
    if (!property_expr_groupGroupCompliant(data)) return false;
    property_expr_ = data;
    return true;
  }

  virtual uint32_t VpiType() const final { return vpiCasePropertyItem; }

  virtual case_property_item* DeepClone(BaseClass* parent, CloneContext* context) const override;
  ...
  virtual  UHDM_OBJECT_TYPE UhdmType() const final { return UHDM_OBJECT_TYPE::uhdmcase_property_item; }

protected:
  void DeepCopy(case_property_item* clone, BaseClass* parent, CloneContext* context) const;
