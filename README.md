...
| **[Caliptra](https://github.com/chipsalliance/caliptra-rtl)** | Caliptra root-of-trust SoC: VeeR EL2 core, AXI sub / manager, mailbox, SHA-256/384/512, HMAC, ECC, Ascon, the ML-DSA / ML-KEM post-quantum block and the caliptra_tlul fabric (`test/caliptra_chip/`) | **867** modules / **172** direct instances of `caliptra_top` | **16 / 21 instances formally proven** vs read_slang (local run; first nightly pending). `read_uhdm` + `hierarchy -check` clean (327k cells, 2:24); read_slang elaborates the same wrapper in 17 s. Required 3 Surelog preprocessor fixes ([#4179](https://github.com/chipsalliance/Surelog/pull/4179)) and 3 frontend fixes; the 5 open instances are a modport-field mismatch, a double driver and one counterexample | [Sweep caliptra](https://github.com/alainmarcel/uhdm2rtlil/actions/workflows/sweep-caliptra.yml) |
