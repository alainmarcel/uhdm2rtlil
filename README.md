...
| **[Caliptra](https://github.com/chipsalliance/caliptra-rtl)** | Caliptra root-of-trust SoC: VeeR EL2 core, AXI sub / manager, mailbox, SHA-256/384/512, HMAC, ECC, Ascon, the ML-DSA / ML-KEM post-quantum block and the caliptra_tlul fabric (`test/caliptra_chip/`) | **867** modules / **172** direct instances of `caliptra_top` | **19 / 21 instances formally proven** vs read_slang (local run; pcr_vault1 needs a 96-minute SAT, so the per-miter budget is 7200 s). `read_uhdm` + `hierarchy -check` clean (327k cells, 2:24); read_slang elaborates the same wrapper in 17 s. Required 3 Surelog preprocessor fixes ([#4179](https://github.com/chipsalliance/Surelog/pull/4179)), a Surelog parameter-default fix ([#4180](https://github.com/chipsalliance/Surelog/pull/4180)) and 13 frontend fixes; the 2 open instances are key_vault1 (SAT-hard at seq 2 since its register block became fully driven; 300-cycle co-sim shows a 2-cycle write-response difference under triage) and rvtop (a struct-member size cast that Surelog types as the whole struct — fixed in [Surelog #4181](https://github.com/chipsalliance/Surelog/pull/4181), bump pending). Full-chip Verilator co-sim of the flat wrapper: read_slang tracks the RTL; read_uhdm differs on 2 of 301 cycles (one SoC AXI write response), down from 270 before the AXI write-path fixes | [Sweep caliptra](https://github.com/alainmarcel/uhdm2rtlil/actions/workflows/sweep-caliptra.yml) |
