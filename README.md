...
| **[Caliptra](https://github.com/chipsalliance/caliptra-rtl)** | Caliptra root-of-trust SoC: VeeR EL2 core, AXI sub / manager, mailbox, SHA-256/384/512, HMAC, ECC, Ascon, the ML-DSA / ML-KEM post-quantum block and the caliptra_tlul fabric (`test/caliptra_chip/`) | **867** modules / **172** direct instances of `caliptra_top` | **20 / 21 instances formally proven** vs read_slang (local run; pcr_vault1 needs a 96-minute SAT, so the per-miter budget is 7200 s). `read_uhdm` + `hierarchy -check` clean (327k cells, 2:24); read_slang elaborates the same wrapper in 17 s. Required 3 Surelog preprocessor fixes ([#4179](https://github.com/chipsalliance/Surelog/pull/4179)), a Surelog parameter-default fix ([#4180](https://github.com/chipsalliance/Surelog/pull/4180)), a struct-member size-cast fix ([#4181](https://github.com/chipsalliance/Surelog/pull/4181)) and 14 frontend fixes — the last one, an element select on a packed 2-D interface member through a modport port (VeeR's ICCM bank addresses), closed rvtop. The 1 open instance is key_vault1 (SAT-hard at seq 2 since its register block became fully driven; 300-cycle co-sim shows a 2-cycle write-response difference under triage). Per-instance Verilator co-sim (nightly, 300 cycles): 16 / 17 comparable instances PASS. Full-chip co-sim of the flat wrapper: read_slang tracks the RTL; read_uhdm differs on 2 of 301 cycles (one SoC AXI write response), down from 270 before the AXI write-path fixes | [Sweep caliptra](https://github.com/alainmarcel/uhdm2rtlil/actions/workflows/sweep-caliptra.yml) |
